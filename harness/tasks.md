...
AGENTS.md 已填写：项目描述、技术栈（Markdown + Shell）、3 条铁律。
...
rules.md 已填写：3 条原则、命名约定表、3 条结构规则、2 条禁止项。
...
#### TASK-007: 修复 check.sh C1/C4 — 跨平台兼容
**完成时间：** 2026-06-26
**产出：** [check.sh](./scripts/check.sh) — harness + template 同步
C1 只统计 IN_PROGRESS\|BLOCKED，C4 用 POSIX find -mtime 替代 stat。

#### TASK-008: CI 加 macos-latest 矩阵
**完成时间：** 2026-06-26
**产出：** [check.yml](../.github/workflows/check.yml)
ubuntu-latest + macos-latest 交叉验证跨平台兼容。
...
#### TASK-010: 修复 check.sh C1 grep -c 退出码陷阱
**完成时间：** 2026-06-26
**产出：** [check.sh](./scripts/check.sh) — harness + template 同步
grep -c → grep \| wc -l，解决无匹配时 exit 1 + || echo 0 叠加问题。

#### TASK-011: 分析 — C1 sed 范围修复 + 自进化流程优化
**完成时间：** 2026-06-26
**产出：** check.sh / AGENTS.md — harness + template 同步
C1 sed 范围终点修正（^### 待办区 → ^## 待办区）+ 外部学习强制化 + 7 步重排序 + 必要性等级 + 建议分级输出。
