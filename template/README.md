...
Harness-Engineering 定义了一套结构化 Markdown 文件（AGENTS → SESSION → tasks → specs / rules / ...），AI 启动时按固定链路逐级读取，确保每次会话都有完整的上下文、明确的目标、统一的规范。
...
| 文件 | 作用 | 维护者 |
|------|------|--------|
| [AGENTS](./AGENTS.md) | AI 入口：身份、铁律、文件清单、检查项、自进化流程 | 用户 |
| [会话状态](./SESSION.md) | 本次会话目标、断点、遗留问题、临时备忘 | 用户 |
| [任务追踪](./tasks.md) | 任务清单：进行中 / 待办 / 已归档，含分支跳转指引 | 用户 |
| [编码规范](./rules.md) | 原则、命名、结构规则、禁止项、已知例外 | 用户 |
| [项目目标](./specs.md) | 核心目标、成功标准、明确不做的事、硬约束 | 用户 |
| [项目概览](./README.md) | 项目介绍、框架特色、读取链路图、项目结构 | 用户 |
| [一致性检查](./scripts/check.sh) | 自动校验任务上限、索引完整、核心文件、新鲜度 | 用户 |
| [Git Hook](./scripts/pre-commit) | 文档变更时自动触发一致性检查 | 用户 |
| [决策记录](./DECISIONS.md) | 记录"为什么这么做"，避免重复讨论 | 智能体 |
| [提示词库](./PROMPTS.md) | 经验证的 AI 提示词模板，按场景复用 | 智能体 |
| [复盘记录](./feedback.md) | 踩坑记录与重复模式，同类问题不再犯 | 智能体 |
| [产出索引](./outputs/INDEX.md) | 所有产出文件的统一登记目录 | 智能体 |
